...
function getKeys() {
  ...
  (process.env.GEMINI_API_KEY || '').split(',').forEach(s => push(s.trim()));
...
async function loadBrain(host) {
  const now = Date.now();
  if (brainCache && now - brainCacheTime < BRAIN_TTL) return brainCache;

  try {
    const base = 'https://' + host;
    const [insRes, knRes] = await Promise.all([
      fetch(base + '/instructions.txt'),
      fetch(base + '/knowledge.txt')
    ]);

    const instructions = insRes.ok ? (await insRes.text()).trim() : '';
    const knowledge = knRes.ok ? (await knRes.text()).trim() : '';

    brainCache = [
      'You are Quix. Follow these instructions exactly.',
      '',
      '=== INSTRUCTIONS ===',
      instructions,
      '',
    ...
    return brainCache || 'You are Quix.';
  }
}
...
module.exports = async function handler(req, res) {
  ...
  if (body.mode === 'tts') {
    ...
    const requested = String(body.model || DEFAULT_TTS_MODEL).trim();
    const modelChain = [requested, ...TTS_MODEL_CHAIN].filter((m, i, a) => m && a.indexOf(m) === i);
  ...
  const brain = await loadBrain(req.headers.host);
